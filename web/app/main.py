...
class ERPApp:
    ...
    def __init__(self, page: ft.Page):
        ...
        self.http_client = httpx.Client(
            base_url=BACKEND_URL, timeout=2.0, trust_env=False
        )
        ...
        if not self.check_backend_health():
            print(
                "ERRO: Backend não está respondendo. Verifique se o servidor está rodando em http://localhost:8000"
            )
            ...
            self.page.add(
                ft.Column(
                    [
                        ...
                        ft.Text(
                            f"Verifique se o servidor está rodando em {BACKEND_URL}/docs",
                            style="bodyLarge",
                        ),
                    ]
                )
    ...
    def check_backend_health(self) -> bool:
        ...
        try:
            ...
            response = self.http_client.get(
                "/health"
            )  # Usando o novo endpoint de health check
            print(
                f"DEBUG: Resposta do backend: status={response.status_code}, content={response.text[:200]}"
            )
    ...
    def on_navigation_change_refresh(self, e):
        ...
        if (
            self.selected_index == destinations_count - 1
        ):  # Índice do botão de sincronização
    ...
    def build_dashboard_view(self) -> ft.Column:
        ...
        def on_filter_click(e):
            ...
            try:
                ...
                # Filtra por data
                filtered = [tx for tx in transactions if start <= tx["date"] <= end]
        ...
        async def load_categories():
            ...
            try:
                ...
                content_container.content = ft.Column(
                    [ft.ProgressRing(), ft.Text("Carregando categorias...")],
                ...
                if not self.categories:
                    ...
                    content_container.content = ft.Column(
                        [
                            ft.Text(
                                "Nenhuma categoria encontrada no banco", color="red"
                            ),
                            ft.FilledButton(
                ...
                content_container.content = ft.ListView(
                    controls=categories_controls, spacing=10, padding=10, expand=True
                )
                print("Atualizando UI")
                self.page.update()
                ...
                content_container.content = ft.Column(
                    [
                        ft.FilledButton(
                            "Erro na conexão. Tentar novamente",
                ...
                self.fiscal_items_table.rows = [
                    ft.DataRow(
                        cells=[
                            ...
                            ft.DataCell(
                                ft.Text(f"{item['note_date']} - {item['seller_name']}")
                            ),
                        ]
                    )
                    for item in items
...
if __name__ == "__main__":
    ft.run(
        main, port=8081
    )  # Executa a aplicação na porta 8081 para evitar conflitos de cache
