...
def test_complete_item_lifecycle(client):
    ...
    assert categories_response.status_code == 200, (
        f"Failed to get categories: {categories_response.text}"
    )
    ...
    # Step 1b: Create a seller
    seller_response = client.post(
        "/sellers", json={"name": "Supermercado Real", "tax_id": "12345678000199"}
    )
    assert seller_response.status_code == 201, (
        f"Failed to create seller: {seller_response.text}"
    )
    ...
    assert mapping_response.status_code == 200, (
        f"Failed to create product mapping: {mapping_response.text}"
    )
    mapping_data = mapping_response.json()
    # The endpoint returns a success message and ID, not the mapping data itself
    assert (
        "mapeamento criado com sucesso" in str(mapping_response.text).lower()
        or "mapeamento atualizado com sucesso" in str(mapping_response.text).lower()
    )
    ...
    assert fiscal_note_response.status_code == 200, (
        f"Failed to create fiscal note: {fiscal_note_response.text}"
    )
    ...
    assert fiscal_item_response.status_code == 200, (
        f"Failed to create fiscal item: {fiscal_item_response.text}"
    )
    ...
    assert "product_ean" in fiscal_item_data, (
        "product_ean field is missing from fiscal item response"
    )
    ...
    print(
        "Integration test passed: The system correctly mapped the fiscal item to the registered product EAN!"
    )
